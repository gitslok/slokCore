...
/*
 * This contract centralizes Slok's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the SLOKToken contract.
 */
contract SlokMaster is Ownable, ISlokMaster {
    ...
    ISlokToken private immutable _slokToken; // Address of the SLOK token contract
    ...
    constructor(ISlokToken slokToken_, uint256 startTime_) {
        require(
            address(slokToken_) != address(0),
            "SlokMaster: slokToken cannot be set to zero address"
        );
        require(
            _currentBlockTimestamp() < startTime_ &&
                startTime_ >= slokToken_.lastEmissionTime(),
            "SlokMaster: invalid startTime"
        );
        _slokToken = slokToken_;
        startTime = startTime_; // Must be set with the same time as SlokToken emission start
    }
    ...
    /*
     * @dev Returns SlokToken address
     */
    function slokToken() external view override returns (address) {
        return address(_slokToken);
    }

    /*
     * @dev Returns SlokToken's emission rate (allocated to this contract)
     */
    function emissionRate() public view returns (uint256) {
        return _slokToken.masterEmissionRate();
    }
    ...
    function _safeRewardsTransfer(
        ...
        uint256 slokBalance = _slokToken.balanceOf(address(this));
    ...
    function _updatePool(address poolAddress) internal {
        ...
        if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
            // calculate how much SLOK rewards are expected to be received for this pool
            uint256 rewards = currentBlockTimestamp
            .sub(lastRewardTime).mul(emissionRate()).mul(allocPoint).div( // nbSeconds
                    totalAllocPoint
                );

            // claim expected rewards from the token
            // use returns effective minted amount instead of expected amount
            (rewards) = _slokToken.claimMasterRewards(rewards);
