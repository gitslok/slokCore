...
contract SlokFactory is ISlokFactory {
    ...
    uint public ownerFeeShare = 30000; // default value = 30%
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "SlokFactory: caller is not the owner");
        _;
    }
    ...
    function createPair(
        ...
        require(tokenA != tokenB, "SlokFactory: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "SlokFactory: ZERO_ADDRESS");
        require(
            getPair[token0][token1] == address(0),
            "SlokFactory: PAIR_EXISTS"
        ); // single check is sufficient
        bytes memory bytecode = type(SlokPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), "SlokFactory: FAILED");
        SlokPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "SlokFactory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), "SlokFactory: zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(
            ...
            "SlokFactory: not setStableOwner"
        );
        require(_setStableOwner != address(0), "SlokFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(
            newOwnerFeeShare > 0,
            "SlokFactory: ownerFeeShare mustn't exceed minimum"
        );
        require(
            newOwnerFeeShare <= OWNER_FEE_SHARE_MAX,
            "SlokFactory: ownerFeeShare mustn't exceed maximum"
        );
    ...
    function setReferrerFeeShare(
        ...
        require(referrer != address(0), "SlokFactory: zero address");
        require(
            referrerFeeShare <= REFERER_FEE_SHARE_MAX,
            "SlokFactory: referrerFeeShare mustn't exceed maximum"
        );
    ...
    function initCode() public pure returns (bytes32) {
        bytes memory bytecode = type(SlokPair).creationCode;
