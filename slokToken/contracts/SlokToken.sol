...
/*
 * SLOK is Slok's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract SlokToken is Ownable, ERC20("Slok token", "SLOK"), ISlokToken {
    ...
    modifier onlyMaster() {
        require(
            ...
            "SlokToken: caller is not the master"
        );
        _;
    }
    ...
    /**
     * @dev Burns "amount" of SLOK by sending it to BURN_ADDRESS
     */
    ...
    function updateAllocations(uint256 farmingAllocation_) external onlyOwner {
        ...
        uint256 totalAllocationsSet = farmingAllocation;
        require(
    ...
    /**
     * @dev Updates SLOK emission rate per second
     *
     * Must only be called by the owner
     */
