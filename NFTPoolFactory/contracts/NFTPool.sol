 ...
 * spNFTs are yield-generating positions when the NFTPool contract has allocations from the Slok Master
 */
contract NFTPool is
    ReentrancyGuard,
    INFTPool,
    ERC721("Slok staking position NFT", "spNFT")
{
    ...
    struct StakingPosition {
        ...
        uint256 boostPoints; // Allocated veSLOK from yieldboost contract (optional)
        uint256 totalMultiplier; // lockMultiplier + allocated veSLOK boostPoints multiplier
        uint256 pendingVeSlokRewards; // Not harvested veSlok rewards
        uint256 pendingSlokRewards; // Not harvested Slok rewards
    }
    ...
    ISlokMaster public master; // Address of the master
    ...
    ISlokPair private _pair;
    IERC20 private _slokToken; // SlokToken contract's address
    IVeSlokToken private _veSlokToken; // VeSlokToken contract's address
    ...
    uint256 public constant MAX_GLOBAL_MULTIPLIER_LIMIT = 50000; // 500%, high limit for maxGlobalMultiplier (100 = 1%)
    uint256 public constant MAX_LOCK_MULTIPLIER_LIMIT = 20000; // 200%, high limit for maxLockMultiplier (100 = 1%)
    uint256 public constant MAX_BOOST_MULTIPLIER_LIMIT = 30000; // 300%, high limit for maxBoostMultiplier (100 = 1%)
    uint256 private _maxGlobalMultiplier = 30000; // 300%
    ...
    uint256 private _maxBoostMultiplier = 20000; // 200%, Max boost that can be earned from veSlok yieldBooster

    uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for veSlokRewardsShare
    uint256 public veSlokRewardsShare = 8000; // 80%, directly defines slokShare with the remaining value to 100%
    ...
    function initialize(
        ...
        _lpToken = lpToken;
        master = master_;
        _slokToken = slokToken;
        _veSlokToken = veSlokToken;
        initialized = true;
        _pair = ISlokPair(address(_lpToken));
        // to convert SLOK to veSLOK
        _slokToken.approve(address(veSlokToken), type(uint256).max);
        router = IUniswapV2Router01(router_);
        _lpToken.approve(address(router), type(uint256).max);
        IERC20(_pair.token0()).approve(router_, type(uint256).max);
        IERC20(_pair.token1()).approve(router_, type(uint256).max);
    }
    ...
    event SetVeSlokRewardsShare(uint256 veSlokRewardsShare);
     ...
     */
    function getMultiplierByBoostPoints(
        uint256 amount,
        uint256 boostPoints
     ...
     */
    function getMultiplierByLockDuration(
        uint256 lockDuration
    ) public view returns (uint256) {
    ...
    function pendingRewards(uint256 tokenId) external view returns (uint256) {
        ...
        if (
            ...
            accRewardsPerShare = accRewardsPerShare.add(
                tokenRewards.mul(1e18).div(_lpSupplyWithMultiplier)
            );
        }
        ...
        return
            position
                ...
                .add(position.pendingVeSlokRewards)
                .add(position.pendingSlokRewards);
    }
     ...
     */
    function setLockMultiplierSettings(
        uint256 maxLockDuration,
        uint256 maxLockMultiplier
    ) external {
        ...
        require(
            maxLockMultiplier <= MAX_LOCK_MULTIPLIER_LIMIT &&
                maxLockMultiplier.add(_maxBoostMultiplier) <=
                _maxGlobalMultiplier,
            "too high"
        );
     ...
     * Must only be called by the owner
     */
    function setBoostMultiplierSettings(
        uint256 maxGlobalMultiplier,
        uint256 maxBoostMultiplier
    ) external {
        _requireOnlyOwner();
        ...
        require(
            maxBoostMultiplier <= MAX_BOOST_MULTIPLIER_LIMIT &&
                maxBoostMultiplier.add(_maxLockMultiplier) <=
                maxGlobalMultiplier,
            "too high"
        );
        // setLockSettings: maxGlobalMultiplier is too high
        ...
        emit SetBoostMultiplierSettings(
            maxGlobalMultiplier,
            maxBoostMultiplier
        );
    }
    ...
    /**
     * @dev Set the share of veSLOK for the distributed rewards
     * The share of SLOK will incidently be 100% - veSlokRewardsShare
     *
     * Must only be called by the owner
     */
    function setVeSlokRewardsShare(uint256 veSlokRewardsShare_) external {
        _requireOnlyOwner();
        require(veSlokRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

        veSlokRewardsShare = veSlokRewardsShare_;
        emit SetVeSlokRewardsShare(veSlokRewardsShare_);
    }
    ...
    function createPosition(
        ...
        uint256 amountWithMultiplier = amount.mul(lockMultiplier.add(1e4)).div(
            1e4
        );
        ...
        _stakingPositions[currentTokenId] = StakingPosition({
            ...
            pendingSlokRewards: 0,
            pendingVeSlokRewards: 0
        });

        // update total lp supply
        _lpSupply = _lpSupply.add(amount);
        _lpSupplyWithMultiplier = _lpSupplyWithMultiplier.add(
            amountWithMultiplier
        );
     ...
     */
    function _addToPosition(uint256 tokenId, uint256 amountToAdd) internal {
    ...
    function addToPosition(
        ...
        uint256 preBalanceA = IERC20(_pair.token0()).balanceOf(address(this));
        uint256 preBalanceB = IERC20(_pair.token1()).balanceOf(address(this));
        uint256 preLpBlance = _pair.balanceOf(address(this));

        IERC20(_pair.token0()).safeTransferFrom(
            msg.sender,
            address(this),
            amountA
        );
        IERC20(_pair.token1()).safeTransferFrom(
            msg.sender,
            address(this),
            amountB
        );
        ...
        router.addLiquidity(
            ...
            0,
            address(this),
            _currentBlockTimestamp() + 1
        );
        _addToPosition(
            tokenId,
            _pair.balanceOf(address(this)).sub(preLpBlance)
        );
        IERC20(_pair.token0()).safeTransfer(
            msg.sender,
            IERC20(_pair.token0()).balanceOf(address(this)).sub(preBalanceA)
        );
        IERC20(_pair.token1()).safeTransfer(
            msg.sender,
            IERC20(_pair.token1()).balanceOf(address(this)).sub(preBalanceB)
        );
    }
    ...
    function withdrawFromPosition(
        ...
        _removeLiquidity(nftOwner, amountToWithdraw);
        _checkOnWithdraw(nftOwner, tokenId, amountToWithdraw);
    }

    function withdrawFromPositionETH(
        uint256 tokenId,
        uint256 amountToWithdraw
    ) external nonReentrant {
        _requireOnlyApprovedOrOwnerOf(tokenId);
        ...
        address nftOwner = ERC721.ownerOf(tokenId);
        _withdrawFromPosition(nftOwner, tokenId, amountToWithdraw);
        _removeLiquidityETH(nftOwner, amountToWithdraw);
        _checkOnWithdraw(nftOwner, tokenId, amountToWithdraw);
    }
    ...
    function emergencyWithdraw(uint256 tokenId) external nonReentrant {
        ...
        require(
            _unlockOperators.contains(msg.sender) ||
                position.startLockTime.add(position.lockDuration) <=
        ...
        _lpSupplyWithMultiplier = _lpSupplyWithMultiplier.sub(
            position.amountWithMultiplier
        );
        ...
        _removeLiquidity(msg.sender, amount);
    }

    function emergencyWithdrawETH(uint256 tokenId) external nonReentrant {
        _requireOnlyOwnerOf(tokenId);
    ...
    function _updatePool() internal {
        ...
        if (rewards > 0) {
            _accRewardsPerShare = _accRewardsPerShare.add(
                rewards.mul(1e18).div(_lpSupplyWithMultiplier)
            );
        }
     ...
     * Users should still be able to deallocate veSLOK from the YieldBooster contract
     */
    ...
    function _removeLiquidityETH(address to, uint256 removeAmount) internal {
        router.removeLiquidityETH(
            _pair.token0() == router.WETH() ? _pair.token1() : _pair.token0(),
            removeAmount,
            0,
            0,
            to,
    ...
    function _removeLiquidity(address to, uint256 removeAmount) internal {
     ...
     */
    function _updateBoostMultiplierInfoAndRewardDebt(
        StakingPosition storage position
    ) internal {
        // keep the original lock multiplier and recompute current boostPoints multiplier
        uint256 newTotalMultiplier = getMultiplierByBoostPoints(
            position.amount,
            position.boostPoints
        ).add(position.lockMultiplier);
        if (newTotalMultiplier > _maxGlobalMultiplier)
            newTotalMultiplier = _maxGlobalMultiplier;

        position.totalMultiplier = newTotalMultiplier;
        uint256 amountWithMultiplier = position
            .amount
            .mul(newTotalMultiplier.add(1e4))
            .div(1e4);
        // update global supply
        _lpSupplyWithMultiplier = _lpSupplyWithMultiplier
            .sub(position.amountWithMultiplier)
            .add(amountWithMultiplier);
        position.amountWithMultiplier = amountWithMultiplier;

        position.rewardDebt = amountWithMultiplier.mul(_accRewardsPerShare).div(
            1e18
        );
    }
    ...
    function _harvestPosition(uint256 tokenId, address to) internal {
        ...
        uint256 pending = position
            .amountWithMultiplier
            .mul(_accRewardsPerShare)
            .div(1e18)
            .sub(position.rewardDebt);
        ...
        if (
            pending > 0 ||
            position.pendingVeSlokRewards > 0 ||
            position.pendingSlokRewards > 0
        ) {
            uint256 veSlokRewards = pending.mul(veSlokRewardsShare).div(
                _TOTAL_REWARDS_SHARES
            );
            uint256 slokAmount = pending.add(position.pendingSlokRewards).sub(
                veSlokRewards
            );

            veSlokRewards = veSlokRewards.add(position.pendingVeSlokRewards);

            // Stack rewards in a buffer if to is equal to address(0)
            if (address(0) == to) {
                position.pendingVeSlokRewards = veSlokRewards;
                position.pendingSlokRewards = slokAmount;
            } else {
                // convert and send veSLOK + SLOK rewards
                position.pendingVeSlokRewards = 0;
                position.pendingSlokRewards = 0;

                if (veSlokRewards > 0)
                    veSlokRewards = _safeConvertTo(to, veSlokRewards);
                // send share of SLOK rewards
                slokAmount = _safeRewardsTransfer(to, slokAmount);

                // forbidden to harvest if contract has not explicitly confirmed it handle it
                _checkOnNFTHarvest(to, tokenId, slokAmount, veSlokRewards);
            }
        }
    ...
    function _lockPosition(uint256 tokenId, uint256 lockDuration) internal {
        ...
        if (endTime > currentBlockTimestamp) {
            require(
                lockDuration >= endTime.sub(currentBlockTimestamp) &&
                    lockDuration > 0,
                "invalid"
            );
        }
    ...
    function _safeRewardsTransfer(
        ...
        uint256 balance = _slokToken.balanceOf(address(this));
        // cap to available balance
        if (amount > balance) {
            amount = balance;
        }
        _slokToken.safeTransfer(to, amount);
        return amount;
    }

    /**
     * @dev Safe convert SLOK to veSLOK function, in case rounding error causes pool to not have enough tokens
     */
    function _safeConvertTo(
        address to,
        uint256 amount
    ) internal returns (uint256) {
        uint256 balance = _slokToken.balanceOf(address(this));
        ...
        if (amount > 0) _veSlokToken.convertTo(amount, to);
        return amount;
    }
     ...
     */
    function _checkOnAddToPosition(
        address nftOwner,
        uint256 tokenId,
        uint256 lpAmount
    ) internal {
    ...
    function _checkOnWithdraw(
        ...
        if (nftOwner.isContract()) {
            bytes memory returndata = nftOwner.functionCall(
                abi.encodeWithSelector(
                    INFTHandler(nftOwner).onNFTWithdraw.selector,
                    msg.sender,
                    tokenId,
                    lpAmount
                ),
