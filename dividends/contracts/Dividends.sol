...
/*
 * This contract is used to distribute dividends to users that allocated veSlok here
 *
 ...
 * This distribution slot is the source of the dividends distribution to veSlok allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IVeSlokTokenUsage, IDividends {
    ...
    address public immutable veSlokToken; // veSlokToken contract

    mapping(address => uint256) public usersAllocation; // User's veSlok allocation
    ...
    constructor(address veSlokToken_, uint256 startTime_) {
        require(veSlokToken_ != address(0), "zero address");
    ...
    /**
     * Allocates "userAddress" user's "amount" of veSlok to this dividends contract
     *
     * Can only be called by veSlokToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IveSlokTokenUsage)
     */
    function allocate(
        address userAddress,
        uint256 amount,
    ...
    /**
     * Deallocates "userAddress" user's "amount" of veSlok allocation from this dividends contract
     *
     * Can only be called by veSlokToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IveSlokTokenUsage)
     */
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no veSlok is allocated or initial distribution has not started yet
        if (
            totalAllocation == 0 ||
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userVeSlokAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userVeSlokAllocation.mul(accDividendsPerShare).div(1e18).sub(
                user.rewardDebt
            )
        );

        user.pendingDividends = 0;
        user.rewardDebt = userVeSlokAllocation.mul(accDividendsPerShare).div(
            1e18
        );
