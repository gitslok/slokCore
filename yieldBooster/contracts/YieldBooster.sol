...
/*
 * This contract is a veSLOK Usage (plugin) that can boost spNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the veSLOKToken contract
 */
    ...
    IYieldBooster
{
    ...
    IVeSlokToken public immutable veSlokToken; // VeSlokToken contract
    ...
    /**
     * @dev Checks if caller is the VeSlokToken contract
     */
    modifier veSlokTokenOnly() {
        require(
            msg.sender == address(veSlokToken),
            "veSlokTokenOnly: caller should be veSlokToken"
        );
        _;
    }
    ...
    /**
     * @dev Returns total veSLOK allocated to this contract by "userAddress"
     */
    ...
    /**
     * @dev Returns total veSLOK allocated to this contract by "poolAddress"
     */
    ...
    /**
     * @dev Returns allocated veSLOK to "tokenId" spNFT from "poolAddress" NFTPool by "userAddress"
     */
    ...
    /**
     * Allocates "userAddress" user's "amount" of veSLOK to this usage contract
     * "data" should contain tokenId and poolAddress
     *
     * Can only be called by VeSlokToken contract, which is trusted to verify amounts
     */
    function allocate(
        address userAddress,
        uint256 amount,
        bytes calldata data
    ) external override nonReentrant veSlokTokenOnly {
        ...
        // allocated veSLOK is added (as boost points) to spNFT
        INFTPool(poolAddress).boost(tokenId, amount);
    }

    /**
     * Deallocates "userAddress" user's "amount" of veSLOK from this usage contract
     * "data" should contain tokenId and poolAddress
     *
     * Can only be called by VeSlokToken contract, which is trusted to verify amounts
     */
    function deallocate(
        ...
        // should only be called if spNFT has not been burned, to avoid having stuck veSLOK on it
        if (INFTPool(poolAddress).exists(tokenId)) {
            // allocated veSLOK is removed (as boost points) from the spNFT
     ...
     * The pool should remove the allocated veSLOK (boost points) from its own spNFT when calling this function
     */
    function deallocateAllFromPool(
        ...
        // update user's veSLOK allocations balance
        veSlokToken.deallocateFromUsage(userAddress, amount);
    }

    /**
     * Deallocates msg.sender's "amount" of veSLOK from VeSlokToken, without adjusting this contract allocations balances
     *
    ...
    function forceDeallocate() external nonReentrant {
        ...
        uint256 amount = veSlokToken.usageAllocations(
            msg.sender,
            address(this)
        );

        // update user's veSLOK allocations balance
        veSlokToken.deallocateFromUsage(msg.sender, amount);
    }
    ...
    /**
     * @dev Returns multiplier that should be applied to a spNFT based on its boost points (allocated veSLOK)
     *
     ...
     * 1% of the pool's allocated veSLOK
     *
    ...
    /**
     * @dev Allocates "userAddress" user's "amount" of veSLOK to "tokenId" spNFT of "poolAddress" pool
     */
    function _allocate(
        address userAddress,
        address poolAddress,
    ...
    /**
     * @dev Deallocates "userAddress" user's "amount" of veSLOK allocated to "tokenId" spNFT of "poolAddress" pool
     */
    function _deallocate(
        ...
        require(
            ...
            "deallocate: not enough allocated veSlok"
        );
